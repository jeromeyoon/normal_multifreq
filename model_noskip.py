...
class DCGAN(object):
	...
	net  = networks(64,self.df_dim)
       	self.nondetail_G,self.detail_G = net.multi_freq_generator_noskip(self.nondetail_images,self.detail_images) 
	...
	self.detail_d_loss_real = binary_cross_entropy_with_logits(tf.random_uniform(self.detail_D[-1].get_shape(),minval=0.7,maxval=1.2,dtype=tf.float32,seed=0),self.detail_D[-1])
        ...
        self.detail_d_loss_real_sum = tf.summary.scalar("detail_d_loss_real",self.detail_d_loss_real)
        self.detail_d_loss_fake_sum = tf.summary.scalar("detail_d_loss_fake",self.detail_d_loss_fake)
        self.detail_d_loss_sum = tf.summary.scalar("detail_d_loss",self.detail_d_loss)
	...
	if self.loss == 'L1':
	   ...
	   self.L_loss_sum = tf.summary.scalar("L2_loss",self.L_loss)
        ...
        self.detail_g_sum = tf.summary.scalar("high_g_loss",self.detail_g_loss)
        ...
        self.detail_gen_loss = self.detail_g_loss + self.g_loss + (self.detail_L_loss+self.L_loss+self.ang_loss)*self.lambda_g_d
	...
	self.g_sum = tf.summary.merge([self.g_loss_sum,self.nondetail_g_sum,self.detail_g_sum,self.L_loss_sum,self.ang_loss_sum,self.nondetail_gen_loss_sum,self.detail_gen_loss_sum])
	...
	counter = 1;
		...
		batch_images = np.array(batches[:,:,:,0]).astype(np.float32)
	        ...
	        batch_low_images = batch_low_images.reshape([self.batch_size,self.ir_image_shape[0],self.ir_image_shape[1],1])
                ...
                _,_,summary,d_err = self.sess.run([nondetail_d_optim,detail_d_optim,self.d_sum,self.d_loss],feed_dict={self.nondetail_images:batch_low_images,self.detail_images:batch_high_images,self.images:batch_images,\
