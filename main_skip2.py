...
from model_queue_skip2 import DCGAN
...
def main(_):
    ...
    if not os.path.exists('./logs_multifreq_skip'):
        os.makedirs('./logs_multifreq_skip')
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_config)) as sess:
		...
		savepath ='./result/skip2_result/Light3/L2ang'
		...
		load,counter = dcgan.load(FLAGS.checkpoint_dir)
		savepath ='./result/skip2_result/allviews/L2ang/%06d' %iteration
                if load:
	            print(" Load Success")
	            for idx in range(len(list_val)):
		        if not os.path.exists(os.path.join(savepath,'%03d' %list_val[idx])):
