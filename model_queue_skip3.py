...
################### ATTENTIION ################################################
####### This model contains all three adversarial loss                    #####
###############################################################################
class DCGAN(object):
	...
	################ Discriminator Loss ######################
	if self.pair: 
            iself.nondetail_D = net.discriminator_low(tf.concat(3,[self.nondetail_images,self.nondetail_normal]),self.keep_prob)
	...
	self.ang_loss = ang_loss.ang_error(self.G,self.normal_images)
        self.nondetail_gen_loss = self.nondetail_g_loss+self.g_loss + (self.nondetail_L_loss+self.L_loss+self.ang_loss)*self.lambda_low
        self.detail_gen_loss = self.detail_g_loss+self.g_loss + (self.detail_L_loss)+(self.L_loss+self.ang_loss)*self.lambda_high
	...
	detail_d_optim = tf.train.AdamOptimizer(config.learning_rate,beta1=config.beta1) \
                          .minimize(self.detail_d_loss, global_step=global_step1,var_list=self.detail_d_vars)
	
	nondetail_d_optim = tf.train.AdamOptimizer(config.learning_rate,beta1=config.beta1) \
                          .minimize(self.nondetail_d_loss, global_step=global_step2,var_list=self.nondetail_d_vars)
	d_optim = tf.train.AdamOptimizer(config.learning_rate,beta1=config.beta1) \
                          .minimize(self.d_loss, global_step=global_step3,var_list=self.d_vars)
        nondetail_g_optim = tf.train.AdamOptimizer(config.learning_rate,beta1=config.beta1) \
                          .minimize(self.nondetail_gen_loss, global_step=global_step4,var_list=self.nondetail_g_vars)

        detail_g_optim = tf.train.AdamOptimizer(config.learning_rate,beta1=config.beta1) \
                          .minimize(self.detail_gen_loss, global_step=global_step5,var_list=self.detail_g_vars)
	...
	if self.use_queue:
		...
		for idx in xrange(0,batch_idxs):
        	     start_time = time.time()
		     _,_,_,d_err =self.sess.run([d_optim,detail_d_optim,nondetail_d_optim,self.d_loss],feed_dict={self.keep_prob:self.dropout})
