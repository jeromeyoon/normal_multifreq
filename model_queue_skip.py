...
class DCGAN(object):
	...
	if not self.use_queue:
		...
		#q = tf.FIFOQueue(4000,[tf.float32,tf.float32,tf.float32,tf.float32 ,tf.float32],[[self.ir_image_shape[0],self.ir_image_shape[1],1],[self.ir_image_shape[0],self.ir_image_shape[1],1],[self.normal_image_shape[0],self.normal_image_shape[1],3],[self.normal_image_shape[0],self.normal_image_shape[1],3],[self.normal_image_shape[0],self.normal_image_shape[1],3]])
	...
	if self.pair: 
	    ...
	    #self.D = net.discriminator(tf.concat(3,[self.ir_images,self.normal_images]),self.keep_prob)
	    #self.D_  = net.discriminator(tf.concat(3,[self.ir_images,self.G]),self.keep_prob,reuse=True)
	    ...
	    #self.D = net.discriminator(self._normal_images,self.keep_prob)
	...
	#d_optim = tf.train.AdamOptimizer(config.d_learning_rate,beta1=config.beta1) \
        #                  .minimize(self.d_loss, global_step=global_step5,var_list=self.d_vars)
